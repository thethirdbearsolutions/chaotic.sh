...
Grouped by subject, in the order `chaotic_mcp_tools/tools/__init__.py`
registers them; the canonical descriptions and schemas are in
`docs/mcp-toolset-schema.json`. The table is generated
(`cd cli && uv run python scripts/gen_mcp_tool_table.py --write`) and
`tests/test_mcp_toolset_sync.py` fails when it is stale.

<!-- mcp-tool-table: generated by scripts/gen_mcp_tool_table.py, do not edit by hand -->
