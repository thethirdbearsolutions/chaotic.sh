/**
 * Revision history viewer for documents and issue descriptions (CHT-1243).
 *
 ...
 * single body field internally via the adapter's bodyField.
 *
...
export async function showRevisionHistory(entityType, entityId) {
  ...
  if (!adapter) {
    showApiError('open revision history', new Error(`Unknown entity type: ${entityType}`));
    return;
  }
  ...
  try {
    ...
    showApiError('load revision history', e);
    return;
  }
  ...
  if (state.viewVersion != null) {
    try {
      ...
      if (token === stateToken) showApiError('load revision', e);
    }
  }
...
async function selectRevision(version) {
  ...
  if (!myState.cache.has(version)) {
    try {
      ...
      if (state === myState && stateToken === myToken) showApiError('load revision', e);
      return;
    }
  }
...
function toggleCompare() {
  ...
  if (myState.mode === 'view') {
    ...
    Promise.all(toLoad.map((v) => myState.adapter.fetchOne(myState.entityId, v).then((snap) => {
      ...
      .then(() => {
        if (state !== myState || stateToken !== myToken) return;
        if (firstError) showApiError('load revisions to compare', firstError);
        renderViewer();
      });
  } else {
    myState.mode = 'view';
