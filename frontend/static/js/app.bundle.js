...
`}];function Tn(e=null){const t=e||U()||"";nr([]);const n=Y().map(o=>`
          ...
          <div class="comments-list">${S.length===0?'<div class="comments-empty">No comments yet</div>':S.map(B=>{var K,H;return`
            <div class="comment" data-comment-id="${u(B.id)}">
              ...
              <div class="comment-body">
                ...
                <div class="comment-content markdown-body">${Be(B.content)}</div>
              </div>
            </div>
          `}).join("")}</div>
  ...
  `,N()}async function op(e,t){var s;const n=document.getElementById("link-issue-results");if(!e||e.length<2){n.innerHTML='<p class="empty-state-small">Enter a search term to find issues</p>';return}try{const i=(s=T())==null?void 0:s.id,a=await v.searchIssues(i,e);if(a.length===0){n.innerHTML='<p class="empty-state-small">No issues found</p>';return}n.innerHTML=a.map(o=>`
    ...
    `,N()}catch(s){E("load labels",s)}}async function up(e,t){try{await v.addLabelToDocument(e,t),R(),$("Label added!","success"),await He(e,!1)}catch(n){E("add label",n)}}async function pp(e,t){try{await v.removeLabelFromDocument(e,t),$("Label removed!","success"),await He(e,!1)}catch(n){E("remove label",n)}}X({"view-document":(e,t)=>{e.preventDefault(),He(t.documentId)},"toggle-doc-selection":(e,t)=>{Yu(t.docId)},"clear-doc-search":()=>{qu()},"clear-doc-project-filter":()=>{Ou()},"clear-all-doc-filters":()=>{Hu()},"show-bulk-move-modal":()=>{Xu()},"bulk-delete-documents":()=>{Ju()},"select-all-docs":()=>{Zu()},"clear-doc-selection":()=>{br()},"exit-selection-mode":()=>{Ki()},"enter-selection-mode":()=>{vr()},"handle-bulk-move":e=>{Qu(e)},"unlink-document-issue":(e,t)=>{lp(t.documentId,t.issueId)},"show-link-issue-modal":(e,t)=>{ap(t.documentId)},"add-document-comment":(e,t)=>{cp(e,t.documentId)},"remove-label-from-doc":(e,t)=>{pp(t.documentId,t.labelId)},"show-add-label-to-doc-modal":(e,t)=>{dp(t.documentId)},"show-edit-document-modal":(e,t)=>{wr(t.documentId)},"delete-document":(e,t)=>{sp(t.documentId)},"create-document":e=>{tp(e)},"update-doc-sprint-dropdown":(e,t,n)=>{ip(t.sprintSelect,n.value)},"update-document":(e,t)=>{np(e,t.documentId)},"search-issues-to-link":(e,t,n)=>{op(n.value,t.documentId)},"link-to-issue":(e,t)=>{rp(t.documentId,t.issueId)},"add-label-to-doc":(e,t)=>{up(t.documentId,t.labelId)}});let Yt=[],Ss={},Ls=new Set,lt=null,Zi=null,Xi=[],Bn=[],Qi=[];function kr(){return Ss}function mp(){return Zi}function gp(){return lt}bt(e=>{e==="currentProject"&&A()==="sprints"&&jn()});async function jn(){const e=U();if(!e){const n=document.getElementById("sprints-list");n&&(n.innerHTML=`
        ...
        `).join(""));try{await v.getCurrentSprint(e),Yt=await v.getSprints(e),fp(),await Cs()}catch(n){t&&(t.innerHTML='<div class="empty-state"><h3>Failed to load sprints</h3></div>'),E("load sprints",n)}}function fp(){const e=document.getElementById("sprints-list");if(!e)return;const t=Yt.find(a=>a.status==="active"),n=Yt.find(a=>a.status==="planned"),s=Yt.filter(a=>a.status==="completed");let i="";if(t){const a=t.budget?`${t.points_spent||0} / ${t.budget} points`:"No budget set",o=t.budget&&(t.points_spent||0)>t.budget;i+=`
    ...
    `).join("")}function sm(e){qe(e),M("issues")}function Dr(){document.getElementById("modal-title").textContent="Create Project",document.getElementById("modal-content").innerHTML=`
...
>
    ...
    `,N(),document.getElementById("create-epic-form").addEventListener("submit",Tg),document.getElementById("create-epic-title").focus()}async function Tg(e){e.preventDefault();const t=document.getElementById("create-epic-project").value,n=document.getElementById("create-epic-title").value.trim(),s=document.getElementById("create-epic-description").value.trim();if(!t){$("Please select a project","error");return}if(!n){$("Please enter a title","error");return}try{const i=await v.createIssue(t,{title:n,description:s||null,issue_type:"epic"});R(),$(`Created epic ${i.identifier}`,"success"),Ta()}catch(i){E("create epic",i)}}async function al(e){try{let t;if(e.includes("-")?t=await v.getIssueByIdentifier(e):t=await v.getIssue(e),t){if(t.issue_type!=="epic"){G(t.id,!1);return}await ol(t.id,!1)}else M("epics",!1)}catch{M("epics",!1)}}async function ol(e,t=!0){try{const[n,s,i,a]=await Promise.all([v.getIssue(e),v.getSubIssues(e),v.getActivities(e),v.getComments(e)]);if(n.issue_type!=="epic"){G(e,t);return}t&&history.pushState({epicId:e,view:A()},"",`/epic/${n.identifier}`),document.querySelectorAll(".view").forEach(w=>w.classList.add("hidden"));const o=document.getElementById("epic-detail-view");o.classList.remove("hidden");const r=A()||"epics",d=Y().find(w=>w.id===n.project_id),c=n.assignee_id?_n(n.assignee_id):null,l=c?Lt(c):null,f=s.length,m=s.filter(w=>w.status==="done"||w.status==="canceled").length,h=f>0?Math.round(m/f*100):0;o.querySelector("#epic-detail-content").innerHTML=`
        ...
        `).join(""));try{_t=await v.getIssues({project_id:e}),xt()}catch(n){t&&(t.innerHTML='<div class="empty-state"><h3>Failed to load board</h3></div>'),E("load board",n)}}function xt(){const e=document.getElementById("kanban-board");e&&(e.innerHTML=rl.map(t=>{const n=_t.filter(s=>s.status===t.key);return`
