...
`}];function En(e=null){const t=e||F()||"";Vo([]);const n=Z().map(o=>`
  ...
  `}).join("")}async function Cr(e){if(!be)return;let t=[];try{t=await v.getRitualGroups(be)}catch{}document.getElementById("modal-title").textContent="Create Ritual",document.getElementById("modal-content").innerHTML=`
