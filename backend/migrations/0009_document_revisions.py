"""Add document_revisions table for auto-versioning document content (CHT-1243).
...
Hand-written like 0006-0008 (makemigrations diffs against the replayed
chain and emits unrelated drift), and rehearsed against a copy of a
real install's DB before landing.
...
def upgrade(ctx):
    ...
    # (Mirrors app.oxyde_models.document.OxydeDocumentRevision Meta.indexes.)
    ctx.execute("""
...
def downgrade(ctx):
    ...
    restore from a pre-0009 backup if you need the snapshots.
    """
